...
GLuint HistSOMWidget::sharedObject = 0;
HistSOMWidget::HistSOMWidget( int timerInterval, QWidget *parent, QGLWidget *shareWidget):QtSOMWidget(0, parent, shareWidget)
{
	pbo = 0;
	log_pbo = 0;
	split_pbo = 0;
...
const char* HistSOMWidget::readShader(std::string name)
{
	std::ifstream file;
...
void HistSOMWidget::setupCuda(ORDERED_MATRIX<MATRIX_TYPE, HOST, COLUMN_MAJOR> &ww,
...
void HistSOMWidget::initSplitPBO()
{
    if (split_pbo) {
        // delete old buffer
...
void HistSOMWidget::createShader()
{
...
void HistSOMWidget::initializeGL()
{
...
//	glEnable (GL_LIGHT0);
	if(som.RUN_DISPLAY){
		...
		cutilSafeCall(cudaMalloc((void**)&d_log_output, sizeof(unsigned int) * 512 * 512));
		cutilSafeCall(cudaMalloc((void**)&d_hist_output, IMAGE_XxY*sizeof( unsigned int ) * 3 ));
	}

}

void HistSOMWidget::resizeGL( int x, int y )
{
    ...
    glLoadIdentity();
    glOrtho(0, 1, 0, 2, 0, 1.0);
}

void HistSOMWidget::paintGL()
{
...
GLuint HistSOMWidget::createObject()
{
	GLuint list = glGenLists(1);

	glNewList(list, GL_COMPILE);
	std::cout << "list " << list << ", " << glGetError() << std::endl;
	glBindBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, pbo);
	glBindTexture  (GL_TEXTURE_TYPE, displayRegTex);
	glPixelStorei  (GL_UNPACK_ALIGNMENT, 1);
	glTexSubImage2D(GL_TEXTURE_TYPE,0, 0, 0, width/2, height/2, GL_RGBA, GL_UNSIGNED_BYTE, 0);
...
void HistSOMWidget::keyPressEvent( QKeyEvent *e )
{
...
void HistSOMWidget::mousePressEvent(QMouseEvent *e)
{
    anchor = e->pos();
}

void HistSOMWidget::mouseMoveEvent(QMouseEvent *e)
{
