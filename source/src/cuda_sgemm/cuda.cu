...
__global__ void reduce(uint *ret, uint *indices, float *ww_sum, const float *vec, const float *data, int index)
{
	...
	if (threadIdx.x < 16)
		ww_sum[ argmax[0] * 16 + threadIdx.x] += data[index * 16 + threadIdx.x];//ww_sum[ 410 * 16 + threadIdx.x] = data[threadIdx.x];
